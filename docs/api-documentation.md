...
- **인증**: 현재 미적용
- **상태**: ✅ **완성** - 포켓몬 검색 및 월드컵 API 모두 구현 완료

## 📋 API 목록

### 1. 월드컵 관련 API (✅ 완성 - 7개 엔드포인트)

#### 1.1 참가자 조회 API
```
POST /api/worldcup/participate
```
...
#### 1.3 월드컵 결과 조회
```
GET /api/worldcup/result/{tournamentId}
```
...
#### 1.5 세대별 월드컵 통계 조회
```
GET /api/worldcup/statistics/generation/{generation}
```

**경로 파라미터:**
| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| generation | Integer | Y | 세대 번호 (0 = 전체 세대) |
...
#### 1.6 타입별 월드컵 통계 조회
```
GET /api/worldcup/statistics/type/{type}
```

**경로 파라미터:**
| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| type | String | Y | 포켓몬 타입 (fire, water, electric 등) |

#### 1.7 세대별+타입별 월드컵 통계 조회
```
GET /api/worldcup/statistics/generation/{generation}/type/{type}
```

**경로 파라미터:**
| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| generation | Integer | Y | 세대 번호 |
| type | String | Y | 포켓몬 타입 |

#### 1.8 자동 월드컵 생성
```
POST /api/worldcup/auto-generate
```
